...
import { convertedMarks } from '../students/students.component';
...
export class ReportComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.dataStorageService.fetchStudents1().pipe(take(1), map((data) => {
      ...
      if (this.student != undefined) {
        ...
        this.isFailed = this.checkFailed(this.student.marksArray);
        this.validPdf = true;
  ...
  checkFailed(marksArray:convertedMarks[]):string{
    for (const key in marksArray) {
      if (Object.prototype.hasOwnProperty.call(marksArray, key)) {
        if(marksArray[key].status == 'failed')
        {
          return 'failed';
        }
      }
  ...
  async generatePDF() {
    ...
    const ObtainedMarks = this.student?.marksArray.reduce((accumulator, currentValue)=>{return (accumulator + currentValue.marks)}, 0);
      ...
      content: [
        {
          ...
          stack: [
            ...
            { text: 'Patel Faliya, Nana Varachha,\n Surat-395010.', style: 'subheader', alignment: 'center' },
            {
              ...
              margin: [0, -60, 0, 0] // Space between logo and text
            }
          ],
        ...
        { color: '#14293f', text: 'Report Card', style: 'sectionHeader', bold: true, fontSize: 15 },
        {
          ...
          layout: {
            ...
            paddingTop: () => 3,
            paddingBottom: () => 3,
            hLineWidth: (rowIndex: number) => (rowIndex === (+this.student!.marksArray.length)+1 ? 1: null),
            vLineWidth: () => 0.5,
            hLineColor: () => '#cfd8dc',
            vLineColor: () => '#cfd8dc',
          }
        ...
        { text: '', height: 10},
        { color: '#14293f', text: 'Overall Result', style: 'sectionHeader', bold: true, fontSize: 15 },
        ,{
          color: '#14293f',
          style: 'table',
          table: {
            ...
            widths: [200,'*'],
            body: [
              [{ text: 'Total Marks', bold: true, style: 'tableHeader'}, { text: ((+this.student!.marksArray.length) * 100), style: 'tableCell'}],
              [{text: 'Obtained Marks', bold: true, style: 'tableHeader'},{text: ObtainedMarks, style: 'tableCell'}
              ],
              [{text: 'Percentage', bold: true, style: 'tableHeader'},{text: ((ObtainedMarks!)/this.student!.marksArray.length).toFixed(2) + '%', style: 'tableCell'}
              ],
              [{text: 'Status', bold: true, style: 'tableHeader'},{text: this.isFailed, style: 'tableCell'}]
              // [Object.values(this.student!.marksArray[1])],
              // [Object.values(this.student!.marksArray[2])],
              // [Object.values(this.student!.marksArray[3])],
              // [Object.values(this.student!.marksArray[4])],
          ...
          border: [true, true, true, true],
          layout: {
            fillColor: (rowIndex: number) => (rowIndex === 0 ? 'white' : (rowIndex % 2 !== 0 ? '#e0f7fa' : null)),
            paddingLeft: () => 6,
            paddingRight: () => 6,
            paddingTop: () => 3,
            paddingBottom: () => 3,
